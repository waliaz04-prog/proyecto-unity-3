...
public class PlayerUpgradeHandler : MonoBehaviour
{
    ...
    [Header("Efectos visuales al comprar")]
    [Tooltip("Lista de efectos disponibles (objetos hijos del jugador, ej. distintos anillos bajo los pies). Cada MaquinaExpendedora elige cuál de estos activar indicando su posición en esta lista (0, 1, 2...).")]
    [SerializeField] private EfectoCompra[] efectosCompra;

    private Coroutine[] corrutinasEfecto;
    ...
    private void Awake()
    {
        ...
        if (efectosCompra != null)
        {
            corrutinasEfecto = new Coroutine[efectosCompra.Length];
            for (int i = 0; i < efectosCompra.Length; i++)
            {
                if (efectosCompra[i].objeto != null)
                    efectosCompra[i].objeto.SetActive(false);
            }
        }
    }

    // Lo llama MaquinaExpendedora al completar una compra, indicando qué efecto
    // de la lista 'Efectos Compra' le corresponde a esa máquina.
    public void MostrarEfectoCompra(int indice)
    {
        if (efectosCompra == null || indice < 0 || indice >= efectosCompra.Length) return;

        GameObject objeto = efectosCompra[indice].objeto;
        if (objeto == null) return;

        if (corrutinasEfecto[indice] != null)
            StopCoroutine(corrutinasEfecto[indice]);

        objeto.SetActive(true);
        corrutinasEfecto[indice] = StartCoroutine(OcultarEfectoDespues(indice, efectosCompra[indice].duracion));
    }

    private IEnumerator OcultarEfectoDespues(int indice, float espera)
    {
        yield return new WaitForSeconds(espera);
        if (efectosCompra[indice].objeto != null)
            efectosCompra[indice].objeto.SetActive(false);
        corrutinasEfecto[indice] = null;
    }
