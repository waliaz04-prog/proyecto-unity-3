...
public class HUDManager : MonoBehaviour
{
    ...
    [Header("Barras")]
    [Tooltip("Usa Scrollbar en vez de Slider: el 'size' del Scrollbar representa el porcentaje restante (el 'value' se mantiene fijo en 0 para que la barra se achique desde un lado en vez de moverse).")]
    [SerializeField] private Scrollbar barraVida;
    [SerializeField] private Scrollbar barraEscudo;
    ...
    private void ActualizarBarras()
    {
        ...
        if (vidaPlayer != null && barraVida != null)
        {
            ...
            barraVida.size = Mathf.Lerp(barraVida.size, vidaObjetivo, delta);
        }

        if (escudoPlayer != null && barraEscudo != null)
        {
            escudoObjetivo = escudoPlayer.ObtenerPorcentajeEscudo();
            barraEscudo.size = Mathf.Lerp(barraEscudo.size, escudoObjetivo, delta);
        }
    ...
    public void RefrescarUI()
    {
        if (vidaPlayer != null && barraVida != null)
        {
            barraVida.value = 0f; // fijo: la barra se achica desde el mismo lado, no se desplaza
            barraVida.size = vidaPlayer.ObtenerPorcentajeVida();
        }

        if (escudoPlayer != null && barraEscudo != null)
        {
            barraEscudo.value = 0f;
            barraEscudo.size = escudoPlayer.ObtenerPorcentajeEscudo();
        }
